...
$statusCode = 0
    ...
    Write-Log -Message "Exporting Qlik Certificates for Rim Node Server"
    $certPw = ConvertTo-SecureString $($adminPassword) -AsPlainText -Force
    export-qlikcertificates -machineNames rimNode -includeSecretsKey -certificatePassword $certPw
    Write-Log -Message "Copying Certificates to c:\qlik\certificates\rimNode"
